 ...
 * File formats
 * 
 * Source: $(BASESRC lzma_/container.d)
 ...
 * 
 * Params:
 * strm   = Pointer to lzma_stream that is at least initialized
 *                      with LZMA_STREAM_INIT.
 * preset = Compression preset to use. A preset consist of level
 *                      number and zero or more flags. Usually flags aren't
 *                      used, so preset is simply a number [0, 9] which match
 *                      the options -0 ... -9 of the xz command line tool.
 *                      Additional flags can be be set using bitwise-or with
 *                      the preset level number, e.g. 6 | LZMA_PRESET_EXTREME.
 * check  = Integrity check type to use. See check.h for available
 *                      checks. The xz command line tool defaults to
 *                      LZMA_CHECK_CRC64, which is a good choice if you are
 *                      unsure. LZMA_CHECK_CRC32 is good too as long as the
 *                      uncompressed file is not many gigabytes.
 *
 * Returns:     - LZMA_OK: Initialization succeeded. Use lzma_code() to
 *                encode your data.
 ...
 *
 * Params:
 * preset   =   Compression preset to use. See the description
 *                          in lzma_easy_encoder().
 * check    =   Type of the integrity check to calculate from
 *                          uncompressed data.
 ...
 * filters = Array of filters. This must be terminated with
 *                      filters[n].id = LZMA_VLI_UNKNOWN. See filter.h for
 *                      more information.
 * check   = Type of the integrity check to calculate from
 *                      uncompressed data.
 *
 * Returns:     - LZMA_OK: Initialization was successful.
 *              - LZMA_MEM_ERROR
 *              - LZMA_OPTIONS_ERROR
 *              - LZMA_PROG_ERROR
 */
 ...
 *
 * Note:        The limit calculated by this function applies only to
 ...
 * Params:
 * strm     =   Pointer to properly prepared lzma_stream
 * memlimit =   Memory usage limit as bytes. Use UINT64_MAX
 *                          to effectively disable the limiter.
 * flags    =   Bitwise-or of zero or more of the decoder flags:
 ...
 *
 * Params:
 * memlimit  =  Pointer to how much memory the decoder is allowed
