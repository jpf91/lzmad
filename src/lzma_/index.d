 ...
 *
 * Returns:      On success, a pointer to an empty initialized lzma_index is
 ...
 * allocator     =    Pointer to lzma_allocator, or NULL to
 *                                use malloc()
 * unpadded_size =    Unpadded Size of a Block. This can be
 *                                calculated with lzma_block_unpadded_size()
 *                                after encoding or decoding the Block.
 * uncompressed_size = Uncompressed Size of a Block. This can be
 ...
 * mode  =  Specify what kind of information the caller wants
 *                      to get. See lzma_index_iter_mode for details.
 *
 * Returns:      If next Block or Stream matching the mode was found, *iter
 ...
 *
 * Params:
 * iter   = Iterator that was earlier initialized with
 *                      lzma_index_iter_init().
 * target = Uncompressed target offset which the caller would
 ...
 * src     =  lzma_index to be appended after dest. If this
 *                        function succeeds, the memory allocated for src
 *                        is freed or moved to be part of dest, and all
 *                        iterators pointing to src will become invalid.
 * allocator = Custom memory allocator; can be NULL to use
 *                        malloc() and free().
 *
 * Returns:      - LZMA_OK: lzma_indexes were concatenated successfully.
 ...
 *
 * Returns:      A copy of the lzma_index, or NULL if memory allocation failed.
 */
 ...
 *
 * Returns:      - LZMA_OK: Initialization succeeded, continue with lzma_code().
 *              - LZMA_MEM_ERROR
 *              - LZMA_PROG_ERROR
 */
 ...
 * Params:
 * strm      =  Pointer to properly prepared lzma_stream
 * i         =  The decoded Index will be made available via
 ...
 * memlimit  =  How much memory the resulting lzma_index is
 *                          allowed to require.
 *
 * The valid `action' values for lzma_code() are LZMA_RUN and LZMA_FINISH.
 * It is enough to use only one of them (you can choose freely; use LZMA_RUN
 ...
 *              - LZMA_PROG_ERROR
 *
 * Note:        This function doesn't take allocator argument since all
 ...
 *
 * Params:
 * i         =  If decoding succeeds, *i will point to a new
 *                          lzma_index, which the application has to
 *                          later free with lzma_index_end(). If an error
 *                          occurs, *i will be NULL. The old value of *i
 *                          is always ignored and thus doesn't need to be
 *                          initialized by the caller.
 * memlimit  =  Pointer to how much memory the resulting
